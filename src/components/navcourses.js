...
function CoursesDropDownNavMenu() {
  ...
  return (
    <div className="dropdown courses-drop-down">
      <button className=" dropdown-toggle explore-courses" type="button" data-bs-toggle="dropdown" aria-expanded="false">
        {t("Courses")}
      </button> 
      <ul className="dropdown-menu">
        {programs.map((program, index) => (
          <li key={index}>
            <Link className="dropdown-item" to={program.route}>{program.name}</Link>
          </li>
        ))}
      </ul>
    </div>
